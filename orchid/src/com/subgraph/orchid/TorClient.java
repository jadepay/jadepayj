...
import com.subgraph.orchid.jadepayboard.Jadepayboard;
...
public class TorClient {
	...
	public TorClient(DirectoryStore customDirectoryStore) {
		...
		jadepayboard = new Jadepayboard();
		jadepayboard.addRenderables(circuitManager, directoryDownloader, socksListener);
	}
	...
	public synchronized void start() {
		...
		if(jadepayboard.isEnabledByProperty()) {
			jadepayboard.startListening();
		}
		isStarted = true;
	}
	...
	public synchronized void stop() {
		...
		try {
			socksListener.stop();
			if(jadepayboard.isListening()) {
				jadepayboard.stopListening();
			}
	...
	public void enableJadepayboard() {
		if(!jadepayboard.isListening()) {
			jadepayboard.startListening();
		}
	}
	
	public void enableJadepayboard(int port) {
		jadepayboard.setListeningPort(port);
		enableJadepayboard();
	}
	
	public void disableJadepayboard() {
		if(jadepayboard.isListening()) {
			jadepayboard.stopListening();
		}
	}
