...
import com.subgraph.orchid.jadepayboard.JadepayboardRenderable;
import com.subgraph.orchid.jadepayboard.JadepayboardRenderer;

public class ConnectionCacheImpl implements ConnectionCache, JadepayboardRenderable {
	...
	public void jadepayboardRender(JadepayboardRenderer renderer, PrintWriter writer, int flags) throws IOException {
		if((flags & JADEPAYBOARD_CONNECTIONS) == 0) {
			return;
		}
		printJadepayboardBanner(writer, flags);
	...
	private void printJadepayboardBanner(PrintWriter writer, int flags) {
		final boolean verbose = (flags & JADEPAYBOARD_CONNECTIONS_VERBOSE) != 0;
		if(verbose) {
